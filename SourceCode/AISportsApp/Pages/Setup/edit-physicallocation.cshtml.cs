...
FormName: edit-physicallocation.cshtml
/*****************************************************************************************************************/
...
{
    [Authorize]
    public class edit_physicallocationModel : PageModel
    {
        ...
        public edit_physicallocationModel(IHttpContextAccessor httpContextAccessor)
        {
        ...
        public async Task OnGetAsync()
        {
            ...
            if (response7.IsSuccessStatusCode)
            {
                ...
                Parent_PhysicalLocation_IDs.Insert(0, new SelectListItem { Value = "0", Text = "<-- Select Parent_Physical Location -->" });
            }
            else
            {
                // Handle errors
            ...
            if (_httpContextAccessor.HttpContext != null)
            {
                ...
                // Making a GET request to the API endpoint
                var response = await _httpClient.GetAsync(GlobalConstants.ValidIssuer + "/api/PhysicalLocation/GetPhysicalLocationsByID/" + ID);
                ...
                else
                {
                    ...
                    ModelState.AddModelError(string.Empty, GlobalConstants.ValidIssuer + "/api/PhysicalLocation/GetPhysicalLocationsByID" + " " + HTMLHelper.GetErrorMessage(responseString));
                }
            }
            /*****************************************************************************************************************/
        }
        public async Task<IActionResult> OnPostEditPhysicalLocationAsync()
        {
            // Validate the model
            ...
            try
            {
                ...
                var jsonContent = $@"
{{
""PhysicalLocation_ID"": {PhysicalLocation_ID},
...
""lstPhysicalLocation"": null
                ...
                var response = await _httpClient.PostAsync(GlobalConstants.ValidIssuer + "/api/PhysicalLocation/UpdatePhysicalLocation", content);
                if (response.IsSuccessStatusCode)
                {
                    var data = await response.Content.ReadAsStringAsync();
                }
                else
                {
                    // Handle errors
                    var responseString = await response.Content.ReadAsStringAsync();
                    ModelState.AddModelError(string.Empty, GlobalConstants.ValidIssuer + "/api/PhysicalLocation/UpdatePhysicalLocation" + " " + HTMLHelper.GetErrorMessage(responseString));
                    return Page();
                }
            }
            catch (Exception ex)
