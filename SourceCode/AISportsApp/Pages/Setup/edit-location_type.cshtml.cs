...
FormName: edit-location_type.cshtml
/*****************************************************************************************************************/
...
{
    [Authorize]
    public class edit_location_typeModel : PageModel
    {
        ...
        public edit_location_typeModel(IHttpContextAccessor httpContextAccessor)
        {
        ...
        [BindProperty]
        [Required(ErrorMessage = "Business Entity Type Name is required.")]
        [StringLength(30, ErrorMessage = "Business Entity Type Name cannot be longer than 30 characters")]
        ...
        public async Task OnGetAsync()
        {
            ...
            //{
            //    url3 = GlobalConstants.ValidIssuer + "/api/locationtype/Getlocation_typesByLocationId/" + Loc;
            //}
            //else
            //{
            ...
            if (_httpContextAccessor.HttpContext != null)
            {
                ...
                // Making a GET request to the API endpoint
                var response = await _httpClient.GetAsync(GlobalConstants.ValidIssuer + "/api/Location_Type/GetLocation_TypesByID/" + ID);
                ...
                else
                {
                    ...
                    ModelState.AddModelError(string.Empty, GlobalConstants.ValidIssuer + "/api/Location_Type/GetLocation_TypesByID" + " " + HTMLHelper.GetErrorMessage(responseString));
                }
            }
            /*****************************************************************************************************************/
        }
        public async Task<IActionResult> OnPostEditLocation_TypeAsync()
        {
            // Validate the model
...
""lstLocation_Type"": null
                ...
                var response = await _httpClient.PostAsync(GlobalConstants.ValidIssuer + "/api/Location_Type/UpdateLocation_Type", content);
                if (response.IsSuccessStatusCode)
                {
                    var data = await response.Content.ReadAsStringAsync();
                }
                else
                {
                    // Handle errors
                    var responseString = await response.Content.ReadAsStringAsync();
                    ModelState.AddModelError(string.Empty, GlobalConstants.ValidIssuer + "/api/Location_Type/UpdateLocation_Type" + " " + HTMLHelper.GetErrorMessage(responseString));
                    return Page();
                }
            }
            catch (Exception ex)
