...
namespace AISportsApp.Pages
{
    ...
    public class add_locationModel : PageModel
    {
        ...
        public add_locationModel(IHttpContextAccessor httpContextAccessor)
        {
        ...
        public async Task OnGetAsync()
        {
           ...
           // }
            ...
            if (response7.IsSuccessStatusCode)
            {
                ...
                Parent_Location_IDs.Insert(0, new SelectListItem { Value = "0", Text = "<-- Select Parent_Location -->" });
            }
            else
            {
                // Handle errors
...
""lstLocation"": null
                ...
                var response = await _httpClient.PostAsync(GlobalConstants.ValidIssuer + "/api/Location/AddLocation", content);
                ...
                else
                {
                    ...
                    ModelState.AddModelError(string.Empty, GlobalConstants.ValidIssuer + "/api/Location/AddLocation" + " " + HTMLHelper.GetErrorMessage(responseString));
                    return Page();
                }
            }
            catch (Exception ex)
