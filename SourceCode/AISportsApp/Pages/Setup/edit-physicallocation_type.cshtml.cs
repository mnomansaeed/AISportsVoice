...
FormName: edit-physicallocation_type.cshtml
/*****************************************************************************************************************/
...
{
    [Authorize]
    public class edit_physicallocation_typeModel : PageModel
    {
        ...
        public edit_physicallocation_typeModel(IHttpContextAccessor httpContextAccessor)
        {
        ...
        public async Task OnGetAsync()
        {
            ...
            //{
            //    url3 = GlobalConstants.ValidIssuer + "/api/physicallocation_type/Getphysicallocation_typesByLocationId/" + Loc;
            //}
            //else
            //{
            ...
            if (response3.IsSuccessStatusCode)
            {
                ...
                Parent_PhysicalLocation_Type_IDs.Insert(0, new SelectListItem { Value = "0", Text = "<-- Select Parent Physical Location Type -->" });
            }
            else
            {
                // Handle errors
                var responseString = await response3.Content.ReadAsStringAsync();
                ModelState.AddModelError(string.Empty, GlobalConstants.ValidIssuer + "/api/physicallocationtype/Getphysicallocationtypes" + " " + HTMLHelper.GetErrorMessage(responseString));
            }
            ...
            if (_httpContextAccessor.HttpContext != null)
            {
                ...
                // Making a GET request to the API endpoint
                var response = await _httpClient.GetAsync(GlobalConstants.ValidIssuer + "/api/PhysicalLocation_Type/GetPhysicalLocation_TypesByID/" + ID);
                ...
                else
                {
                    ...
                    ModelState.AddModelError(string.Empty, GlobalConstants.ValidIssuer + "/api/PhysicalLocation_Type/GetPhysicalLocation_TypesByID" + " " + HTMLHelper.GetErrorMessage(responseString));
                }
            }
            /*****************************************************************************************************************/
        }
        public async Task<IActionResult> OnPostEditPhysicalLocation_TypeAsync()
        {
            // Validate the model
...
""lstPhysicalLocation_Type"": null
                ...
                var response = await _httpClient.PostAsync(GlobalConstants.ValidIssuer + "/api/PhysicalLocation_Type/UpdatePhysicalLocation_Type", content);
                if (response.IsSuccessStatusCode)
                {
                    var data = await response.Content.ReadAsStringAsync();
                }
                else
                {
                    // Handle errors
                    var responseString = await response.Content.ReadAsStringAsync();
                    ModelState.AddModelError(string.Empty, GlobalConstants.ValidIssuer + "/api/PhysicalLocation_Type/UpdatePhysicalLocation_Type" + " " + HTMLHelper.GetErrorMessage(responseString));
                    return Page();
                }
            }
            catch (Exception ex)
