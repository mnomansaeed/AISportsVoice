...
{
    [Authorize]
    public class edit_kittypeModel : PageModel
    {
        ...
        public edit_kittypeModel(IHttpContextAccessor httpContextAccessor)
        {
        ...
        public async Task OnGetAsync()
        {
            ...
            if (_httpContextAccessor.HttpContext != null)
            {
                ...
                // Making a GET request to the API endpoint
                var response = await _httpClient.GetAsync(GlobalConstants.ValidIssuer + "/api/KitType/GetKitTypesByID/" + ID);
                ...
                else
                {
                    ...
                    ModelState.AddModelError(string.Empty, GlobalConstants.ValidIssuer + "/api/KitType/GetKitTypesByID" + " " + HTMLHelper.GetErrorMessage(responseString));
                }
            }
            /*****************************************************************************************************************/
        }
        public async Task<IActionResult> OnPostEditKitTypeAsync()
        {
            // Validate the model
...
""lstKitType"": null
                ...
                var response = await _httpClient.PostAsync(GlobalConstants.ValidIssuer + "/api/KitType/UpdateKitType", content);
                if (response.IsSuccessStatusCode)
                {
                    var data = await response.Content.ReadAsStringAsync();
                }
                else
                {
                    // Handle errors
                    var responseString = await response.Content.ReadAsStringAsync();
                    ModelState.AddModelError(string.Empty, GlobalConstants.ValidIssuer + "/api/KitType/UpdateKitType" + " " + HTMLHelper.GetErrorMessage(responseString));
                    return Page();
                }
            }
            catch (Exception ex)
