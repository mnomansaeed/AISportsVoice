...
namespace AISportsApp.Pages
{
    ...
    public class edit_employeeModel : PageModel
    {
        ...
        public edit_employeeModel(IHttpContextAccessor httpContextAccessor)
        {
        ...
        [StringLength(200, ErrorMessage = "Job Title cannot be longer than 200 characters")]
        ...
        public async Task OnGetAsync()
        {
            ...
            if (_httpContextAccessor.HttpContext != null)
            {
                ...
                // Making a GET request to the API endpoint
                var response = await _httpClient.GetAsync(GlobalConstants.ValidIssuer + "/api/Employee/GetEmployeesByID/" + ID);
                ...
                else
                {
                    ...
                    ModelState.AddModelError(string.Empty, GlobalConstants.ValidIssuer + "/api/Employee/GetEmployeesByID" + " " + HTMLHelper.GetErrorMessage(responseString));
                }
            }
            /*****************************************************************************************************************/
        }
...
""lstEmployee"": null
                ...
                var response = await _httpClient.PostAsync(GlobalConstants.ValidIssuer + "/api/Employee/UpdateEmployee", content);
                if (response.IsSuccessStatusCode)
                {
                    var data = await response.Content.ReadAsStringAsync();
                }
                else
                {
                    // Handle errors
                    var responseString = await response.Content.ReadAsStringAsync();
                    ModelState.AddModelError(string.Empty, GlobalConstants.ValidIssuer + "/api/Employee/UpdateEmployee" + " " + HTMLHelper.GetErrorMessage(responseString));
                    return Page();
                }
            }
            catch (Exception ex)
