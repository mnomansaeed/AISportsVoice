...
FormName: edit-subcategory.cshtml
/*****************************************************************************************************************/
...
{
[Authorize]
public class edit_subcategoryModel :  PageModel
{
...
public edit_subcategoryModel(IHttpContextAccessor httpContextAccessor)
{
...
[BindProperty]
[Required(ErrorMessage = "Please select a Asset Type.")]
public string CategoryId { get; set; }
 public List<SelectListItem> CategoryIds { get; set; }
[BindProperty]
[StringLength(1000, ErrorMessage = "Sub Asset Type Name cannot be longer than 1000 characters")]
 public string? SubCategoryName  { get; set; }
[BindProperty]
[StringLength(2000, ErrorMessage = "Desription cannot be longer than 2000 characters")]
...
// Populate dropdown CategoryIds
CategoryIds = new List<SelectListItem>();
string url2 = "";
if (UserRole != "Super Admin Group")
{
url2 = GlobalConstants.ValidIssuer + "/api/category/GetcategoriesByLocationId/" + Loc;
}
else
{
url2 = GlobalConstants.ValidIssuer + "/api/category/Getcategories";
}
// Making a GET request to the API endpoint
...
// Process the data as needed
JObject Jdata = JObject.Parse(data);
// Extract lstcategory array and convert it to a list of category objects
CategoryIds = Jdata["lstCategory"]
 .Select(item => new SelectListItem
{
 Value = (string)item["CategoryId"],
 Text = (string)item["CategoryName"]
}).ToList();
CategoryIds.Insert(0, new SelectListItem { Value = "", Text = "<-- Select CategoryId -->" });
}
else
{
// Handle errors
var responseString = await response2.Content.ReadAsStringAsync();
ModelState.AddModelError(string.Empty,GlobalConstants.ValidIssuer + "/api/category/Getcategories" + " " + HTMLHelper.GetErrorMessage(responseString));
}
...
// Making a GET request to the API endpoint
var response = await _httpClient.GetAsync(GlobalConstants.ValidIssuer + "/api/SubCategory/GetSubCategoriesByID/" + ID);
...
ModelState.AddModelError(string.Empty, GlobalConstants.ValidIssuer + "/api/SubCategory/GetSubCategoriesByID" + " " + HTMLHelper.GetErrorMessage(responseString));
}
}
/*****************************************************************************************************************/
}
...
var response = await _httpClient.PostAsync(GlobalConstants.ValidIssuer + "/api/SubCategory/UpdateSubCategory",content);
if (response.IsSuccessStatusCode)
{
var data = await response.Content.ReadAsStringAsync();
}
...
// Handle errors
var responseString = await response.Content.ReadAsStringAsync();
ModelState.AddModelError(string.Empty,GlobalConstants.ValidIssuer + "/api/SubCategory/UpdateSubCategory" + " " + HTMLHelper.GetErrorMessage(responseString));
return Page();
}
}
catch (Exception ex)
...
// If validation passes, you can proceed with other logic
 ...
 return LocalRedirect("~/Setup/subcategory"); // Redirects to /some/path within the same app
}
}
}
/*****************************************************************************************************************/
