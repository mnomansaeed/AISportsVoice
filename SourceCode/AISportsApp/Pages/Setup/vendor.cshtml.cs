...
public vendorModel(IHttpContextAccessor httpContextAccessor)
{
...
{
var obj = JsonConvert.DeserializeObject<ObjVendorData>(requestBody);
if (obj != null)
{
var id = obj.Id;
...
var response = await _httpClient.PostAsync(GlobalConstants.ValidIssuer + "/api/Vendor/DeleteVendor",content);
...
var responseString = await response.Content.ReadAsStringAsync();
 return new JsonResult(new { success = false, message = GlobalConstants.ValidIssuer + "/api/Vendor/DeleteVendor" + " " + HTMLHelper.GetErrorMessage(responseString) });
}
}
}
}
