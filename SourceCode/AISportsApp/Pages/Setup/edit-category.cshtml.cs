...
namespace AISportsApp.Pages
{
    ...
    public class edit_categoryModel : PageModel
    {
        ...
        public edit_categoryModel(IHttpContextAccessor httpContextAccessor)
        {
        ...
        [BindProperty]
        [Required(ErrorMessage = "Aeet Type Name is required.")]
        [StringLength(800, ErrorMessage = "Asset Type Name cannot be longer than 800 characters")]
        public string CategoryName { get; set; }
        [BindProperty]
        [StringLength(2000, ErrorMessage = "Description cannot be longer than 2000 characters")]
        public string? Description { get; set; }
        [BindProperty]
        [RegularExpression(@"^-?\d+$", ErrorMessage = "Only integer values are allowed.")]
        public Int32? Life { get; set; }
        [BindProperty]
        [StringLength(50, ErrorMessage = "Is Checkout cannot be longer than 50 characters")]
        ...
        public async Task OnGetAsync()
        {
            ...
            if (_httpContextAccessor.HttpContext != null)
            {
                ...
                // Making a GET request to the API endpoint
                var response = await _httpClient.GetAsync(GlobalConstants.ValidIssuer + "/api/Category/GetCategoriesByID/" + ID);
                ...
                else
                {
                    ...
                    ModelState.AddModelError(string.Empty, GlobalConstants.ValidIssuer + "/api/Category/GetCategoriesByID" + " " + HTMLHelper.GetErrorMessage(responseString));
                }
            }
            /*****************************************************************************************************************/
        }
...
""lstCategory"": null
                ...
                var response = await _httpClient.PostAsync(GlobalConstants.ValidIssuer + "/api/Category/UpdateCategory", content);
                if (response.IsSuccessStatusCode)
                {
                    var data = await response.Content.ReadAsStringAsync();
                }
                else
                {
                    // Handle errors
                    var responseString = await response.Content.ReadAsStringAsync();
                    ModelState.AddModelError(string.Empty, GlobalConstants.ValidIssuer + "/api/Category/UpdateCategory" + " " + HTMLHelper.GetErrorMessage(responseString));
                    return Page();
                }
            }
            catch (Exception ex)
            ...
            return LocalRedirect("~/Setup/category"); // Redirects to /some/path within the same app
        }
    }
}
/*****************************************************************************************************************/
