...
FormName: edit-sterilizertype.cshtml
/*****************************************************************************************************************/
...
{
[Authorize]
public class edit_sterilizertypeModel :  PageModel
{
...
public edit_sterilizertypeModel(IHttpContextAccessor httpContextAccessor)
{
...
[BindProperty]
 public Int32 SterilizerTypeId  { get; set; }
[BindProperty]
[Required(ErrorMessage = "Please select a Business Entity.")]
public string Location_ID { get; set; }
 public List<SelectListItem> Location_IDs { get; set; }
[BindProperty]
[StringLength(150, ErrorMessage = "Sterilizer Type Name cannot be longer than 150 characters")]
 public string? SterilizerTypeName  { get; set; }
[BindProperty]
...
// Making a GET request to the API endpoint
var response = await _httpClient.GetAsync(GlobalConstants.ValidIssuer + "/api/SterilizerType/GetSterilizerTypesByID/" + ID);
...
ModelState.AddModelError(string.Empty, GlobalConstants.ValidIssuer + "/api/SterilizerType/GetSterilizerTypesByID" + " " + HTMLHelper.GetErrorMessage(responseString));
}
}
/*****************************************************************************************************************/
}
public async Task<IActionResult> OnPostEditSterilizerTypeAsync()
{
 // Validate the model
...
var jsonContent = $@"
{{
""SterilizerTypeId"": {SterilizerTypeId},
""Location_ID"": {Location_ID},
""SterilizerTypeName"": ""{SterilizerTypeName}"",
...
""User_IP"": ""{User_IP}"",
""Site_Id"": {TenantId},
""lstSterilizerType"": null
}}";
 // Set JSON content for the request
var content = new StringContent(jsonContent, Encoding.UTF8, "application/json");
// Making a POST request to the API endpoint
var response = await _httpClient.PostAsync(GlobalConstants.ValidIssuer + "/api/SterilizerType/UpdateSterilizerType",content);
if (response.IsSuccessStatusCode)
{
var data = await response.Content.ReadAsStringAsync();
}
...
// Handle errors
var responseString = await response.Content.ReadAsStringAsync();
ModelState.AddModelError(string.Empty,GlobalConstants.ValidIssuer + "/api/SterilizerType/UpdateSterilizerType" + " " + HTMLHelper.GetErrorMessage(responseString));
return Page();
}
}
catch (Exception ex)
...
// If validation passes, you can proceed with other logic
 ...
 return LocalRedirect("~/Setup/sterilizertype"); // Redirects to /some/path within the same app
}
}
}
/*****************************************************************************************************************/
