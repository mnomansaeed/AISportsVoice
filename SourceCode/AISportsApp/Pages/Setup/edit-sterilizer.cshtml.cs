...
FormName: edit-sterilizer.cshtml
/*****************************************************************************************************************/
...
{
    [Authorize]
    public class edit_sterilizerModel : PageModel
    {
        ...
        public edit_sterilizerModel(IHttpContextAccessor httpContextAccessor)
        {
        ...
        [BindProperty]
        public Int32 SterilizerId { get; set; }
        [BindProperty]
        [Required(ErrorMessage = "Please select a Business Entity.")]
        public string Location_ID { get; set; }
        public List<SelectListItem> Location_IDs { get; set; }
        [BindProperty]
        [StringLength(150, ErrorMessage = "Sterilizer Name cannot be longer than 150 characters")]
        public string? SterilizerName { get; set; }
        [BindProperty]
        ...
        public async Task OnGetAsync()
        {
            ...
            if (_httpContextAccessor.HttpContext != null)
            {
                ...
                // Making a GET request to the API endpoint
                var response = await _httpClient.GetAsync(GlobalConstants.ValidIssuer + "/api/Sterilizer/GetSterilizersByID/" + ID);
                ...
                else
                {
                    ...
                    ModelState.AddModelError(string.Empty, GlobalConstants.ValidIssuer + "/api/Sterilizer/GetSterilizersByID" + " " + HTMLHelper.GetErrorMessage(responseString));
                }
            }
            /*****************************************************************************************************************/
        }
        public async Task<IActionResult> OnPostEditSterilizerAsync()
        {
            // Validate the model
...
""lstSterilizer"": null
                ...
                var response = await _httpClient.PostAsync(GlobalConstants.ValidIssuer + "/api/Sterilizer/UpdateSterilizer", content);
                if (response.IsSuccessStatusCode)
                {
                    var data = await response.Content.ReadAsStringAsync();
                }
                else
                {
                    // Handle errors
                    var responseString = await response.Content.ReadAsStringAsync();
                    ModelState.AddModelError(string.Empty, GlobalConstants.ValidIssuer + "/api/Sterilizer/UpdateSterilizer" + " " + HTMLHelper.GetErrorMessage(responseString));
                    return Page();
                }
            }
            catch (Exception ex)
            ...
            return LocalRedirect("~/Setup/sterilizer"); // Redirects to /some/path within the same app
        }
    }
}
/*****************************************************************************************************************/
